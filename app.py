...
#%% Datos para las cartas informativas DETALLE DE MATERIALES 

# STOCK TOTAL

detmat1 = dbc.Card([
    dbc.CardBody([
        html.H6("Stock total", className="card-title",id="detmat1"),
        html.P("Stock total"
               
               , className="card-text",id="detmat1")
        ]
     )],
    style={
...
detmat2 = dbc.Card([
    dbc.CardBody([
        html.H6("Fecha estimada fin de material", className="card-title",id="detmat2"),
        html.P("Fecha estimada fin de material"
               
               , className="card-text",id="detmat2")
        ]
     )],
    style={
...
detmat3 = dbc.Card([
    dbc.CardBody([
        html.H6("Prioridad de compra", className="card-title",id="detmat3"),
        html.P("Stock por bodega"
               
               , className="card-text",id="detmat3")
        ]
     )],
    style={
...
detmat4 = dbc.Card([
    dbc.CardBody([
        html.H6("Consumo promedio mensual", className="card-title",id="detmat4"),
        html.P("Consumo promedio mensual"
               
               , className="card-text",id="detmat4")
        ]
     )],
    style={
...
detal = stocktotalbodmas. groupby(['CODIGO', 'DESCRIPCION', 'NOMBREBODEGA', 'STOCKPORBODEGA','Descripcion_cod','UNIDAD' ]).REGISTRO.count().sort_values(ascending=False)
...
def render_page_content(pathname):
    ...
    if pathname == "/":
        return [
            ...
            html.Div([
                html.Button("Descargar     Excel", id="btn_xlsxok"),
                dcc.Download(id="download-dataframe-xlsxok"),

                # 'local' or 'session'
                dcc.Store(id='store-dataok', data=[], storage_type='memory')
            ...
            html.Div([
                dbc.Row([dbc.Col(detmat1, width=3),
                         dbc.Col(detmat2, width=3),
                         dbc.Col(detmat3, width=3),
                         dbc.Col(detmat4, width=3),
                         
                         ], 
                         justify="center")]),  # justify="start", "center", "end", "between", "around"
...
# %% DASH Análisis de transacciones valorizadas
        ...
        return [
            ...
            html.Div(dcc.Graph(id='movmat3'), style={
                     'width': '100%', 'display': 'inline-block'}),
            html.Div(dcc.Graph(id='auto3'), style={
                     'width': '100%', 'display': 'inline-block'}),
...
# %% DASH DROPDOWN ANALISIS DE TRANSACCIONESXXXX
...
def func(n_clicks, jsonified_cleaned_data):
    if n_clicks > 0:
        ...
        return dcc.send_data_frame(ak.to_excel, "Movimientos_bodegas_valorizadasDPCE.xlsx", sheet_name="DPCE")


# %% DASH DROPDOWN ANALISIS DE TRANSACCIONES 
...
@app.callback(
    ...
    Input('dropdownmcp', 'value'),
    Input('dropdownprio', 'value'),
    Input('dropdownmas50', 'value'),
    Input('my-date-picker-range', 'start_date'),
    Input('my-date-picker-range', 'end_date'),
    Input('dropdowngrmas50', 'value'),
    Input('dropdownsonal', 'value'),
    Input('dropdowntipogoi', 'value'),


)
def update_graph(dropdownmat, dropdowncc, dropdownautor, dropdowntipo, dropdownmmi, dropdownmcp, dropdownprio, dropdownmas50, start_date, end_date, dropdowngrmas50, dropdownsonal, dropdowntipogoi):
    #dff = movbod.copy()
    #movibodmasxn['CANTIDAD'] = movibodmasxn['CANTIDAD'].astype(str)
   #movibodmasxn["CANTIDAD2"] = movibodmasxn["CANTIDAD"] + " " + movibodmasxn["UNIDAD"]
    # SUMA
    ...
    if not dropdownprio or 'NA' in dropdownprio:
        ...
        if dropdownprio != "NA":
            ...
            # motivo
    if not dropdownmas50 or 'NA' in dropdownmas50:
        dff = dff
    else:
        if dropdownmas50 != "NA":
            dff = dff[dff['Motivo'].isin(dropdownmas50)]
    # TIPO PERSONAL
    ...
    if not start_date or 'NA' in start_date or not end_date or 'NA' in end_date:
        ...
        if start_date != "NA":
            ...
            # grupo 50%
    if not dropdowngrmas50 or 'NA' in dropdowngrmas50:
        dff = dff
    else:
        if dropdowngrmas50 != "NA":
            dff = dff[dff['Grupomas50'].isin(dropdowngrmas50)]
...
def store_data(dropdownmat, dropdowncc, dropdownautor, dropdowntipo, dropdownmmi, dropdownmcp, dropdownprio, dropdownmas50, start_date, end_date, dropdowngrmas50, dropdownsonal, dropdowntipogoi,):
    # hypothetical enormous dataset with millions of rows

    movbod2 = movbod
    movbod2[['FECHA_']] = movbod2[['FECHA']]

    dffr = movbod2. groupby(['CODIGO', 'Descripcion', 'DOCUMENTO', 'Descripcion_cod', 'BODEGA', 'CENTRO_DE_COSTO', 'Tipopersonal', 'ENTREGA_O_RECIBE', 'AUTORIZADOR',
    ...
    if not dropdownmas50 or 'NA' in dropdownmas50:
        ...
        if dropdownmas50 != "NA":
            dffr = dffr[dffr['Motivo'].isin(dropdownmas50)]
    # TIPO PERSONAL
    if not dropdowntipo or 'NA' in dropdowntipo:
        dffr = dffr
    else:
    ...
    if not dropdowngrmas50 or 'NA' in dropdowngrmas50:
        ...
        if dropdowngrmas50 != "NA":
            dffr = dffr[dffr['Grupomas50'].isin(dropdowngrmas50)]
            # GASTO O INVERSION
    if not dropdowntipogoi or 'NA' in dropdowntipogoi:
        dffr = dffr
    else:
...
def func(n_clicks, jsonified_cleaned_data):
    if n_clicks > 0:
        ...
        #dff = dff[dff['Grupomas50'].isin(dropdowngrmas50)]
        dff2 = dff2. groupby(['CODIGO', 'Descripcion', 'MMI', 'PRIORIZAR_COMPRA', 'Grupomas50', 'CRITICO', 'Descripcion_cod', 'BODEGA', 'ENTREGA_O_RECIBE',
    ...
    return dcc.send_data_frame(dff2.to_excel, "Movimientos_bodegas_DPCE.xlsx", sheet_name="DPCE")
...
def data(dropdownmatok,dropdownprio,dropdownmmi,dropdownmcp):


    dffl = stocktotalbodmas. groupby(['CRITICO','PRIORIZAR_COMPRA','MMI','CODIGO', 'DESCRIPCION', 'NOMBREBODEGA', 'STOCKPORBODEGA','Descripcion_cod','UNIDAD', ]).REGISTRO.count().sort_values(ascending=False)
...
def func(n_clicks, jsonified_cleaned_data):
    if n_clicks > 0:
        ...
        dffl2 = dffl. groupby(['CODIGO', 'DESCRIPCION', 'Descripcion_cod', 'STOCKTOTAL', 'ACTIVIDAD', 'STOCK_MAXIMO', 'STOCK_MINIMO', 'CANTIDAD_SUGERIDAD_PARA_COMPRA', 'PRIORIZAR_COMPRA', 'CRITICO', 'CODIGOCPC', 'FECHAULTIMA_COMPRA',
                                         'FECHA_ULTIMOEGRESO', 'MMI', 'FECHA_PROYECCION_FIN_MATERIAL', 'PORCENTAJE_DE_PRIORIDAD', 'PREC_UNITARIO', 'ESTIMADO_VALOR_DE_COMPRA', 'VALOR_MATERIALES', ]).REGISTRO.count().sort_values(ascending=False)
        ...
        dffl2 = dffl2.reset_index(level='DESCRIPCION')
        dffl2 = dffl2.reset_index(level='CODIGO')
        
        dffl2= dffl2.drop_duplicates(['DESCRIPCION', 'CODIGO'], keep='last')
        #dffl = dff[dff['Grupomas50'].isin(dropdowngrmas50)]

    return dcc.send_data_frame(dffl2.to_excel, "Materiales_DPCE.xlsx", sheet_name="DPCE")
